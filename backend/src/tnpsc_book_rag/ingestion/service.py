...
class IngestionService:
    ...
    async def run_once(self, worker_id: str | None = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                # The worker loop remains alive; the original failure is intentionally not exposed.
    ...
    async def _extract_and_persist(self, work_item: IngestionWorkItem) -> None:
        ...
        with TemporaryDirectory(prefix=f"tnpsc-extraction-{document.id}-") as temporary:
            root = Path(temporary)
